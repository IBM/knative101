...
1. Deploy the application. We will create a Knative Service named fib-knative which will run our fib-knative image on dockerhub. We'll give this service the revision-name `fib-knative-one`. If no revision name is defined, the system will automatically generate a revision name.

    ```
    kn service create fib-knative --image docker.io/ibmcom/fib-knative --revision-name fib-knative-one
    ```
